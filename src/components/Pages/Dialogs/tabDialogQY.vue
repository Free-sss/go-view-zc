<template>
  <n-modal
    v-model:show="status.showTabDialog"
...
const status = reactive({
  showTabDialog: props.showDialog,
...
const handleOnTabTwoOpertionClick = (row: any) => {
  ...
  if (status.selectTabTwoRow && status.selectTabTwoRow.cameras.length > 0) {
    ...
    status.selectedCamera =
      status.selectTabTwoRow.cameras[0].cameraCode || status.selectTabTwoRow.cameras[0].camera_code
  }
}
...
watch(
  ...
  (newData: any) => {
    ...
    if (newData === '1' && props.showTabScene) {
      //场景
      getOrgList()
      getSceneTypeList()
      getSceneInfoList(status.searchOrg1, status.searchSceneType || '')
...
watch(
  ...
  (newData: any) => {
    if (newData) {
      status.showTabDialog = newData
      getOrgList()
      if (props.showTabScene) {
        getSceneTypeList()
        getSceneInfoList(status.searchOrg1, status.searchSceneType || '')
      }
    }
...
const getOrgList = () => {
  axiosInstance({
    method: 'POST',
    //@ts-ignore
    url: `${window.htconfig.API_IP}aw/tvManage/tv/qy/getOrgByUserId`,
...
const getOrgTree = (originalData: any) => {
  ...
  const tree = []
  let pathLenth = !originalData[0] ? 0 : originalData[0].pathName.split('/').length
  // 第一遍遍历：创建基础节点并记录层级关系
  originalData.forEach(item => {
    if (item.pathName.split('/').length < pathLenth) {
      pathLenth = item.pathName.split('/').length
    }
