...
const handleItemClick = (e: MouseEvent, item: CreateComponentType) => {
  // 按下左键 + CTRL
  e.stopPropagation();
  if (window.$KeyboardActive?.ctrl) return
  // 判断左右键
  // expend.value = !expend.value
  mousedownHandleSingle(e, item)
}
...
const mousedownHandleSingle = (e: MouseEvent, item: CreateComponentType) => {
  e.preventDefault()
  e.stopPropagation()
  onClickOutSide()
  // 若此时按下了 CTRL, 表示多选
