...
const selectTarget = computed(() => {
  ...
  console.log(chartEditStore.fetchTargetIndex())
  let target = chartEditStore.componentList[chartEditStore.fetchTargetIndex()]

  if (target?.isGroup && selectId[0] !== target?.id) {
    //选中了分组的子组件
    //@ts-ignore
    target = target.groupList?.find(item => item.id === selectId[0])
    // eslint-disable-next-line vue/no-side-effects-in-computed-properties
  } else if (target?.isGroup && selectId[0] === target?.id) {
